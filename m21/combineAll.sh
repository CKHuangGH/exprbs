...
do
	...
	ssh root@$i chmod 777 /root/exprbs/m21/worker_node.sh
	ssh root@$i sh /root/exprbs/m21/worker_node.sh $cluster &
	cluster=$((cluster+1))
done
...
do
	...
	helm repo update
helm install cilium cilium/cilium --version 1.13.4 --wait --wait-for-jobs --namespace kube-system --set cluster.name=cluster0 --set cluster.id=0
done
