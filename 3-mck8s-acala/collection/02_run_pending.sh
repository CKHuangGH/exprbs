...
do 
	...
	ssh root@$i . /root/exprbs/3-mck8s-acala/collection/script/topnode.sh $j > /dev/null &
	#ssh root@$i . /root/mck8s_vm/sasi/sasi/script/toppodm.sh $j > /dev/null &
	ssh root@$i . /root/exprbs/3-mck8s-acala/collection/script/getpod.sh $j > /dev/null &
	j=$((j+1))	
done
